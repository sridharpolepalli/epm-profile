...
| Client | Create client **`campus-internal-team1`** (public, no client secret). Enable **Standard flow**. Set **Valid redirect URIs**: `https://localhost:7100/authorization`. Set **Web origins**: `https://localhost:7100` |
...
- **UI** – Uses Keycloak at `http://localhost:8080`, realm `epm-realm`, client `campus-internal-team1` (see [UI configuration](#ui-configuration)).
...
| **Client ID** | `campus-internal-team1` | Or the client id they assign |
...
- After client creation, we need:
  ...
  - **Client ID** (e.g. `campus-internal-team1`)
...
const keycloak = new Keycloak({
  ...
  clientId: 'campus-internal-team1'              // Client ID
});
```
...
| Client ID | Same file → `clientId` | `campus-internal-team1` |
